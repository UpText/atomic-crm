...
test("user onboarding", async ({ page, isMobile, menu, dismissToast }) => {
  ...
  await expect(page).toHaveTitle(/UpTextCrm/);
  await expect(page.getByText("Welcome to UpTextCrm")).toBeVisible();
  ...
  await expect(page.getByText("Install UpTextCrm")).toBeVisible();
