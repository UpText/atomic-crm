CREATE PROCEDURE [crmapi].[deal_notes_put](
    @ID VARCHAR(MAX),
    @deal_id INT = NULL,
    @sales_id INT = NULL,
    @date DATETIME2 = NULL,
...
BEGIN
    ...
    BEGIN TRY
        IF @id_int IS NULL
        BEGIN
            RAISERROR('Invalid deal_notes id', 16, 1);
            RETURN 400;
        END

        IF NOT EXISTS (SELECT 1 FROM crm.deal_notes WHERE id = @id_int AND tenant = @auth_tenant)
        BEGIN
            RAISERROR('Unknown deal_notes', 16, 1);
            RETURN 404;
        END
        ...
        UPDATE crm.deal_notes
        SET deal_id = COALESCE(@deal_id, deal_id),
        ...
        EXEC crmapi.deal_notes_get @ID = @ID, @auth_tenant = @auth_tenant;
        RETURN 200;
    END TRY
    BEGIN CATCH
        IF @@TRANCOUNT > 0
