...
UpTextCrm is free and open-source.
...
- ✉️ **Capture Emails**: CC UpTextCrm to automatically save communications as notes.
...
Clone the UpTextCrm repository locally:

```sh
git clone https://github.com/[username]/UpTextCrm.git
```
...
UpTextCrm can also run against the SQL Server-backed SQLWebAPI provider instead of Supabase.
...
UpTextCrm components are published as a Shadcn Registry file. This means you can update your installation by calling the following command:

```sh
npx shadcn add https://www.uptext.com/uptextcrm/r/uptextcrm.json -o
```
...
- The `https://www.uptext.com/uptextcrm/r/uptextcrm.json` file is automatically published by the CI/CD pipeline
